...
- [ ] Utente dedicato al client SUS creato **da Nova**, con ruolo `Sus` e password scelta al momento: `wm-package/src/Nova/AbstractUserResource.php` espone `Password::make()` (riga 66) e `RoleBooleanGroup::make(__('Roles'), 'roles')` (riga 71), entrambi riservati a chi passa `RolesAndPermissionsService::allowsUser()`. Nessun comando artisan di provisioning
